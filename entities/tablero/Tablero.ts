...
class Tablero {
  ...
  init(config: Config) {
    ...
    malla.forEach((elemento, i) =>
      elemento.forEach((casilla, j) => {
        if (i < config.size - 1)
          casilla.nuevoVecino(Vecino.ABAJO, malla[i + 1]![j]!);
        if (i > 0) casilla.nuevoVecino(Vecino.ARRIBA, malla[i - 1]![j]!);
        if (j < config.size - 1)
          casilla.nuevoVecino(Vecino.DERECHA, malla[i]![j + 1]!);
  ...
  dispara(x: number, y: number): boolean {
    ...
    let casillaClickada: Casilla | undefined = this.mapa.find(
      item => item.x === x && item.y === y,
    );

    if (casillaClickada === undefined) {
      return false;
    }

    if (posicionSubmarino.x === x && posicionSubmarino.y === y) {
      casillaClickada.estado = 'tocado';
      return true;
    }

    if (casillaClickada.valor > 0) {
      casillaClickada.estado = 'humo';
    } else {
      casillaClickada.estado = 'agua';
    }

    this.mapa.filter(item => item.valor > 0).forEach(item => (item.valor -= 1));
    let casilla: Casilla | undefined = this.mapa.find(
      item => item.x === posicionSubmarino.x && item.y === posicionSubmarino.y,
    );
